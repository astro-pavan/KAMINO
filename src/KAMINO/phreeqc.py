...
def find_partial_pressure(P: float, T: float, carbon_molality: float):
    ...
    input_file_path_new = f'{PHREEQC_path}/input_partial_pressure'
    wd: str = os.getcwd()
...
def find_carbon_molality(P: float, T: float, P_CO2: float):
    ...
    input_file_path_new = f'{PHREEQC_path}/input_molality'
    wd: str = os.getcwd()

    input_modifications: dict[int, str] = {
        1 : f'DATABASE {wd}/external/phreeqc-3.8.6-17100/database/phreeqc.dat',
        4 : f'    temp        {T + ABSOLUTE_ZERO:.2f}        # Temperature in degrees Celsius',
        5 : f'    pressure    {P / EARTH_ATM:.4f}         # Pressure in atmospheres',
        13 : f'    CO2(g) {np.log10(P_CO2 / EARTH_ATM):.2f}'
    }
...
if __name__ == '__main__':
    ...
    molality = find_carbon_molality(1e5, 300, 0.004 * 1e5) # type: ignore

    print(molality)

    P_CO2, P_H2O = find_partial_pressure(1e5, 300, molality) # type: ignore
